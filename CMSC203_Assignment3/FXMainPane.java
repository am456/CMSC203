...
public class FXMainPane extends BorderPane {
	...
	FXMainPane() {
	    ...
	    decryption.setOnAction(event -> {
			...
			//Added by Angela Minardi
			if (decryptedText.equals("")) //decryptedText returns "" if in bounds test fails
			{
				decryptedTextField4.setText("encryption failed"); //Displays that encryption failed
			}
			else
			{
				decryptedTextField4.setText(decryptedText); //If it passes the bounds test, shows result of decryptedText
			}
		});
